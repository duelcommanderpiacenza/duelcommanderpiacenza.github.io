...
- Every page's `<head>` has an inline synchronous script (before first paint) that sets `data-theme` (dark/light) and adds `html.nav-mobile` if `matchMedia("(max-width: 640px) and (pointer: coarse) and (hover: none)")` matches **or** `window.innerWidth <= 640 && navigator.maxTouchPoints > 0` — the second check is a fallback for OEM Android browsers (Samsung Internet has a history of this) that misreport `hover`/`pointer` and would otherwise never match the media query on a real phone. Same combined check is duplicated in `js/nav-dropdown.js`'s/`admin/js/admin-nav-dropdown.js`'s own self-healing fallback. Most mobile-only CSS is gated on the `html.nav-mobile` class (or the media query directly), not just a width breakpoint.

## Nav & responsive breakpoints

- **900px** — nav switches from wrapping tabs to a single horizontally-scrollable row.
- **640px + (pointer:coarse + hover:none, or real touch hardware)** (`html.nav-mobile`, see above) — nav becomes a floating pill fixed to the bottom of the screen; tapping it opens an upward panel listing all tabs. This is the *only* place safe-area-inset / overscroll-behavior fixes were added — desktop and tablet widths are untouched by that work. Known trade-off: this also now catches a touchscreen Windows laptop resized to a narrow window (previously deliberately excluded, since its primary input is really a mouse/trackpad) — accepted on purpose to fix real phones on Samsung Internet.
