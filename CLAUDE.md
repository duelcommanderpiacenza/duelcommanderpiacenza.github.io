...
- Every page's `<head>` has an inline synchronous script (before first paint) that sets `data-theme` (dark/light) and adds `html.nav-mobile` if `matchMedia("(max-width: 640px) and (pointer: coarse) and (hover: none)")` matches — real touch phones only, not just a narrow window. Most mobile-only CSS is gated on this exact class+media-query combo, not just a width breakpoint.

## Nav & responsive breakpoints

- **900px** — nav switches from wrapping tabs to a single horizontally-scrollable row.
- **640px + pointer:coarse + hover:none** (`html.nav-mobile`) — nav becomes a floating pill fixed to the bottom of the screen; tapping it opens an upward panel listing all tabs. This is the *only* place safe-area-inset / overscroll-behavior fixes were added — desktop and tablet widths are untouched by that work.
